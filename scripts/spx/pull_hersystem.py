...
def osi(exp,cp,k): return f"SPXW  {exp:%y%m%d}{cp}{int(round(k*1000)):08d}"
needm={}
for day,g in spx.groupby("day"):
    ...
    lo=int(round(E*0.988/5)*5); hi=int(round(E*1.012/5)*5)     # +-1.2% band, both C and P (covers ATM long + OTM spreads)
    for k in range(lo,hi+5,5):
        for cp in ("C","P"): needm.setdefault(day[:7],set()).add(osi(exp,cp,k))
